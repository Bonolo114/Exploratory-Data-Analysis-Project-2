...
coaldata<- baltimore[which(baltimore[,"SCC"] %in% coalcodes),]
dim(coaldata)
coaldata
...
barplot(height=Annual_Coal_Emissions$Emissions, names.arg=Annual_Coal_Emissions$year, xlab="years", 
        ylab=expression('total PM'[2.5]*' emission'), col = 8,
        main=expression('Total PM'[2.5]*' from Coal in the Baltimore City'))
dev.off()        
